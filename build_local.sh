...
build_config() {
    ...
    sudo time make O=out CC="ccache clang" CXX="ccache clang++" ARCH=arm64 CROSS_COMPILE=$CLANG_PATH/bin/aarch64-linux-gnu- CROSS_COMPILE_ARM32=$CLANG_PATH/bin/arm-linux-gnueabi- LD=ld.lld $KERNEL_CONFIG | tee config.log
    # Disable LTO
    ...
    # sed -i 's/CONFIG_SECURITY_SELINUX=y/CONFIG_SECURITY_SELINUX=n/' out/.config
...
build_kernel() {
    ...
    # Delete old files
    test -d $KERNEL_PATH/out/arch/arm64/boot && sudo rm -rf $KERNEL_PATH/out/arch/arm64/boot/*
    # Begin compile
    sudo time make O=out $@ CC="ccache clang" CXX="ccache clang++" ARCH=arm64 -j$(nproc) CROSS_COMPILE=$CLANG_PATH/bin/aarch64-linux-gnu- CROSS_COMPILE_ARM32=$CLANG_PATH/bin/arm-linux-gnueabi- LD=ld.lld 2>&1 | tee kernel.log
}
...
echo [INFO] Start build config.
build_config

echo [INFO] Start build kernel.
build_kernel
...
echo [INFO] Start make Anykernel3.zip
make_anykernel3_zip
cd $KERNEL_PATH
